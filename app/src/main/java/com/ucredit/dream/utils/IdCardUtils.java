 ...
 * http://blog.csdn.net/suncaoyong/article/details/8647037
 * 
...
public class IdCardUtils {
    ...
    public static boolean isValid(String idCard) {
        return new IdCardUtils(idCard).validate();
    }

    public boolean validate() {
        if (null == this.cacheValidateResult) {
            boolean result = true;
            result = result && null != this.cardNumber; // 身份证号不能为空
            result = result
                && IdCardUtils.NEW_CARD_NUMBER_LENGTH == this.cardNumber
                    .length(); // 身份证号长度是18(新证)
            // 身份证号的前17位必须是阿拉伯数字
            for (int i = 0; result
                && i < IdCardUtils.NEW_CARD_NUMBER_LENGTH - 1; i++) {
                char ch = this.cardNumber.charAt(i);
                result = result && ch >= '0' && ch <= '9';
            }
            // 身份证号的第18位校验正确
            result = result
                && IdCardUtils.calculateVerifyCode(this.cardNumber) == this.cardNumber
                    .charAt(IdCardUtils.NEW_CARD_NUMBER_LENGTH - 1);
            ...
            try {
                ...
                result = result
                    && birthDate.after(IdCardUtils.MINIMAL_BIRTH_DATE);
                /**
            ...
            this.cacheValidateResult = Boolean.valueOf(result);// TODO
            // 完整身份证号码的省市县区检验规则
        }
        return this.cacheValidateResult;
    }
    ...
    public IdCardUtils(String cardNumber) {
        if (null != cardNumber) {
            ...
            if (IdCardUtils.OLD_CARD_NUMBER_LENGTH == cardNumber.length()) {
                cardNumber = IdCardUtils.contertToNewCardNumber(cardNumber);
            }
        }
    ...
    private int getGenderCode() {
        ...
        char genderCode = this.cardNumber
            .charAt(IdCardUtils.NEW_CARD_NUMBER_LENGTH - 2);
    ...
    private SimpleDateFormat createBirthDateParser() {
        return new SimpleDateFormat(IdCardUtils.BIRTH_DATE_FORMAT);
    }
    ...
    private static char calculateVerifyCode(CharSequence cardNumber) {
        int sum = 0;
        for (int i = 0; i < IdCardUtils.NEW_CARD_NUMBER_LENGTH - 1; i++) {
            char ch = cardNumber.charAt(i);
            sum += (ch - '0') * IdCardUtils.VERIFY_CODE_WEIGHT[i];
        }
        return IdCardUtils.VERIFY_CODE[sum % 11];
    }
    ...
    private static String contertToNewCardNumber(String oldCardNumber) {
        StringBuilder buf = new StringBuilder(
            IdCardUtils.NEW_CARD_NUMBER_LENGTH);
