...
public class JsonStreamerEntity implements HttpEntity {
    ...
    public JsonStreamerEntity(ResponseHandlerInterface progressHandler, 
            boolean useGZipCompression, String elapsedField,String key,String clientId) {
    ...
    public void writeTo(final OutputStream out) throws IOException {
        ...
        OutputStream outputStream = contentEncoding != null
                ? new GZIPOutputStream(out, BUFFER_SIZE): out;
        ...
        String jsonStr = new String(os.toByteArray());
        
        // Flush the contents up the stream.
        os.flush();
        os.close();
        
        outputStream.write(encrypt(jsonStr));
        outputStream.flush();
        AsyncHttpClient.silentCloseOutputStream(outputStream);
    }

    /**
     * ucredit encrypt method
     * @param jsonStr
     * @return
     */
    private byte[] encrypt(String jsonStr) {
        try {
            if(this.key != null){
                String encryptStr = AESUtils.doEncryptCBC(jsonStr, this.key);
                StringBuilder sb = new StringBuilder("UCC:");
            ...
            Log.e(LOG_TAG, "ucredit encrypt failure!");
        }
