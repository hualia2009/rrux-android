...
public class AsyncHttpClient {
    ...
    public static final int DEFAULT_SOCKET_TIMEOUT = 90 * 1000;
     ...
     */
    public AsyncHttpClient(boolean isSSL) {
        this(true, 80, 443,isSSL);
    }
     ...
     */
    public AsyncHttpClient(int httpPort,boolean isSSL) {
        this(true, httpPort, 443,isSSL);
    }
     ...
     */
    public AsyncHttpClient(int httpPort, int httpsPort,boolean isSSL) {
        this(true, httpPort, httpsPort,isSSL);
    }
     ...
     */
    public AsyncHttpClient(boolean fixNoHttpResponseException, int httpPort, int httpsPort,boolean isSSL) {
        this(getDefaultSchemeRegistry(fixNoHttpResponseException, httpPort, httpsPort,isSSL),isSSL);
    }
     ...
     * @param httpsPort                  HTTPS port to be used, must be greater than 0
     */
    private static  SchemeRegistry getDefaultSchemeRegistry(boolean fixNoHttpResponseException, int httpPort, int httpsPort,boolean isSSL) {
        ...
        if (fixNoHttpResponseException) {
            if(isSSL){
                sslSocketFactory = MySSLSocketFactory.getFixedSocketFactory(mContext,UcreditDreamApplication.isDoubleCheck);
            }else{
                sslSocketFactory = MySSLSocketFactory.getFixedSocketFactory(mContext,isSSL);
            }
        } else {
     ...
     */
    public AsyncHttpClient(SchemeRegistry schemeRegistry,boolean isSSL) {
        this.ISSSL = isSSL;
        BasicHttpParams httpParams = new BasicHttpParams();
        
        connectTimeout = UcreditDreamApplication.timeout;
        responseTimeout = UcreditDreamApplication.timeout; //设置超时时间由服务器返回
     ...
     * 添加header
     */
    private void addUcreditHeader(){
        clientHeaderMap.put("Authorization", "Bearer "+ UcreditDreamApplication.token);
        clientHeaderMap.put("X-Auth-Token",UcreditDreamApplication.token);
        clientHeaderMap.put("Client-Id", ""+UcreditDreamApplication.clientId);
        if(!ISSSL){
            clientHeaderMap.put("Token-Source", "uxue-app");
        }
    }
    ...
    protected AsyncHttpRequest newAsyncHttpRequest(DefaultHttpClient client, HttpContext httpContext, HttpUriRequest uriRequest, String contentType, ResponseHandlerInterface responseHandler, Context context) {
        if(this.ISSSL){
        return new AsyncHttpRequest(client, httpContext, uriRequest, responseHandler,UcreditDreamApplication.isDoubleCheck,clientHeaderMap);
        }else{
            return new AsyncHttpRequest(client, httpContext, uriRequest, responseHandler,this.ISSSL,clientHeaderMap);
        }
    }
    ...
    protected RequestHandle sendRequest(DefaultHttpClient client, HttpContext httpContext, HttpUriRequest uriRequest, String contentType, ResponseHandlerInterface responseHandler, Context context) {
        ...
        AsyncHttpRequest request = newAsyncHttpRequest(client, httpContext, uriRequest, contentType, responseHandler, mContext);
