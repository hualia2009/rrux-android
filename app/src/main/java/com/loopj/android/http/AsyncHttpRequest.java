...
public class AsyncHttpRequest implements Runnable {
    ...
    private boolean isdubbocheck = false;
    private Map<String, String> header;

    public AsyncHttpRequest(AbstractHttpClient client, HttpContext context, HttpUriRequest request, ResponseHandlerInterface responseHandler,boolean isdubbocheck,Map<String, String> header) {
        ...
        this.isdubbocheck = isdubbocheck;
        this.header = header;
    }
        ...
        if(!this.isdubbocheck)
            ...
            try {  
                ...
                for (String header : this.header.keySet()) {
                    String value = this.header.get(header);
                    conn.setRequestProperty(header, value);
                }
                HttpEntity he = r.getEntity();
                if(he!=null){
                    if(he.getContentType()!=null)conn.addRequestProperty(he.getContentType().getName(), he.getContentType().getValue());
                    if(he.getContentEncoding()!=null)conn.addRequestProperty(he.getContentEncoding().getName(), he.getContentEncoding().getValue());
                    if(he.getContentLength()>0)conn.addRequestProperty("Content-Length", ""+he.getContentLength());
                ...
                StatusLine s = new BasicStatusLine(r.getProtocolVersion(), conn.getResponseCode(), conn.getResponseMessage());
                response = new BasicHttpResponse(s);
                HttpEntity rhe = new InputStreamEntity(conn.getInputStream(), conn.getContentLength());
                response.setEntity(rhe);
            }catch(Exception e){
                e.printStackTrace();
            }
        }
